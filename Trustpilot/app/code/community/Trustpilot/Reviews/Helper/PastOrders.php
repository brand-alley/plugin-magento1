...
class Trustpilot_Reviews_Helper_PastOrders extends Mage_Core_Helper_Abstract
{
    ...
    {
        $this->_helper->setConfig('sync_in_progress', 'true');
        $this->_helper->setConfig("show_past_orders_initial", 'false');
        try {
            $key = $this->_helper->getKey();
            $collect_product_data = Trustpilot_Reviews_Model_Config::WITHOUT_PRODUCT_DATA;
            if (!is_null($key)) {
                $this->_helper->setConfig('past_orders', 0);
                $pageId = 0;
                $sales_collection = $this->getSalesCollection($period_in_days);
                ...
                while ($post_batch) {
                    ...
                    if (!is_null($post_batch)) {
                        ...
                        $response = $this->_trustpilot_api->postBatchInvitations($key, $batch);
                        ...
                        if ($code < 200 || $code > 202) {
                            $this->_helper->setConfig("show_past_orders_initial", 'true');
                            $this->_helper->setConfig('sync_in_progress', 'false');
                            $this->_helper->setConfig('past_orders', 0);
                            $this->_helper->setConfig('failed_orders', '{}');
                            return;
                        }
                    }
                    $pageId = $pageId + 1;
    ...
    public function resync()
    {
        ...
        try {
            $key = $this->_helper->getKey();
            $failed_orders_object = json_decode($this->_helper->getConfig('failed_orders'));
            $collect_product_data = Trustpilot_Reviews_Model_Config::WITHOUT_PRODUCT_DATA;
            if (!is_null($key)) {
    ...
    {
        $syncInProgress = $this->_helper->getConfig('sync_in_progress');
        $showInitial = $this->_helper->getConfig('show_past_orders_initial');
    ...
    private function getSalesCollection($period_in_days) {
        $date = new DateTime();
        $args = array(
            'date_created' => $date->setTimestamp(time() - (86400 * $period_in_days))->format('Y-m-d'),
            'limit' => 20,
            'past_order_statuses' => json_decode($this->_helper->getConfig('master_settings_field'))->pastOrderStatuses
        );
